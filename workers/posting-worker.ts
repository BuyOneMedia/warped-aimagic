 ...
 * Handles async platform posting jobs queued via the Next.js app
 */
...
async function processPostingJob(job: Job<PostingJobData>): Promise<void> {
  const { platformListingId, productId, userId, platform, listing } = job.data;
...
const worker = new Worker<PostingJobData>(
  ...
  processPostingJob,
  {
    ...
    concurrency: 2, // Process 2 jobs at a time (browser automation is heavy)
  }
);
...
console.log("🚀 AI Magic posting worker started");
